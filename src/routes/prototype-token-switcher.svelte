...
<script lang="ts">
	...
	const variants = [
		...
		{
			key: 'B',
			name: 'C+D hybrid (midpoint brightness/saturation)',
			tokens: { info: '#d6ace6', interactive: '#98c1d4', success: '#a9c394' }
		},
		{
			key: 'C',
	...
	// {@html} in <svelte:head> renders once and never updates — write the
	// style element imperatively so variant switches actually restyle the page.
	$effect(() => {
		let el = document.getElementById('proto-354-style');
		if (!el) {
			el = document.createElement('style');
			el.id = 'proto-354-style';
