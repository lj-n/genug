...
import Fixture from './alert-dialog-form.test-fixture.svelte';

/**
 * A remote-form double for the AlertDialogForm `form` prop: submitting the
 * dialog's form plays `submit` through the primitive's lifecycle.
 */
...
describe('AlertDialogForm — success', () => {
...
describe('AlertDialogForm — cancel', () => {
	it('renders the built-in cancel button and closes without submitting', async () => {
		let submitted = false;
		render(Fixture, {
			props: {
				form: mockForm(() => {
		...
		screen.getByRole('button', { name: m.cancel() }).click();
...
describe('AlertDialogForm — footer', () => {
...
describe('AlertDialogForm — thrown error surface', () => {
