...
- Consume remote queries per the client-side rules in `docs/remote-functions.md` (ADR-0003): query objects at script top level, `await` at the read site; top-level `$derived(await q)` only for values read unconditionally in always-rendered markup.
