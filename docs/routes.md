...
- Prefer async-derived remote data in components: `const foo = $derived(await getFoo(...))` at script top level — never `await` in markup. See the client-side rules in `docs/remote-functions.md` (ADR-0003), including when a wasteful eager fetch warrants extracting a component instead.
